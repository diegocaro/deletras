...
*/
	...
	var $mode = 'oth';
	var $pass_crypt_method   = 'sha1'; // md5, sha1, crypt, crc32,callback
	...
	function login($ignore_cookie = false) // username,password,group
   {
	   ...
	   if(!empty($this->controller->data[$this->user_model]))
	   {
		...
		return $this->_login($params, $ignore_cookie);
   }
	...
	function othLogin($params,$ignore_cookie=false) // username,password,group
	{
		...
		else
		{
			...
			{	
                ...
                $res = $UserModel->saveField($this->user_table_last_visit, date('Y-m-d H:i:s')); 
			}
		...
		case -1:
			{
				 ...
				 return "Nombre de usuario o contraseña vacíos.";
			}break;
		case -2:
			{
				 //return "Wrong ".$this->user_login_var."/".$this->user_passw_var;
				 return "Nombre de usuario o contraseña erróneos.";
			}break;
		case -3:
			{
				 //return "Too many login attempts.";
				 return "Has intentado muchas veces iniciar sesión.";
			}break;
		default:
			{
				 //return "Invalid error ID";
				 return "Error no documentado, por favor cuéntanos que sucedió.";
			}break;
